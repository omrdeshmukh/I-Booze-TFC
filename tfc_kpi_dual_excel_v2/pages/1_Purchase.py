...
ops = pd.concat([df for df in data["OPS"].values()], ignore_index=True, sort=False) if data["OPS"] else pd.DataFrame()
fin = pd.concat([df for df in data["FIN"].values()], ignore_index=True, sort=False) if data["FIN"] else pd.DataFrame()
...
def filt(df):
    ...
    if not sel_sup: return df
    return df[df["supplier"].isin(sel_sup)]
...
if "supplier" in ops.columns and "rejection_pct" in ops.columns:
    with right:
        rej = ops.groupby("supplier")["rejection_pct"].mean().reset_index()
...
if "supplier" in ops.columns and "component_obsolete_pct" in ops.columns:
    with left:
        cob = ops.groupby("supplier")["component_obsolete_pct"].mean().reset_index()
...
if "supplier" in ops.columns and "raw_material_cost_pct" in ops.columns:
    with right:
        rmc = ops.groupby("supplier")["raw_material_cost_pct"].mean().reset_index()
...
if "supplier" in fin.columns and "operating_profit" in fin.columns:
    fig = px.bar(fin.groupby("supplier")["operating_profit"].sum().reset_index(), x="supplier", y="operating_profit", title="Operating Profit by Supplier")
    st.plotly_chart(fig, use_container_width=True)
